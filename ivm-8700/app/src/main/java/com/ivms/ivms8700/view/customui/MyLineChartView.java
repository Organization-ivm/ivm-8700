...
public class MyLineChartView extends View {
    ...
    private int intervalX = 200; // 坐标刻度的间隔

    private int intervalY = 160; // y轴刻度的间隔
    ...
    private int textToXYAxisGap = 50; // xy轴的文字距xy线的距离
    ...
    private float maxValueY = 100; // y轴最大值
...
//        drawLegend(canvas);
    ...
    private void drawY(Canvas canvas) {
        ...
        for (int i = 0; i < yTitles.size(); i++) {
            canvas.drawText(yTitles.get(i), originX - textToXYAxisGap - getTextWidth(paintText, "0"),
                    mHeight - paddingDown - leftRightExtra - i * intervalY + getTextHeight(paintText, "0") / 2, paintText);
        }
        // 截取折线超出部分（右边）
        paintBack.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OVER));
